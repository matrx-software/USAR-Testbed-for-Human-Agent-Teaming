 ...
 */
function draw(state, world_settings, new_messages, accessible_chatrooms, world_completed,
    world_completed_data, new_tick) {
    ...
    if (latest_tick_processed == current_tick) {
        ...
        if (world_completed) {
            ...
            show_completion_screen(world_completed_data);
        }
        return;
    }
    ...
    obj_ids.forEach(function(objID) {
        ...
        var show_busy_condition =  (obj.hasOwnProperty("is_blocked_by_action") &&
                                    obj.hasOwnProperty('visualize_when_busy') &&
                                    obj['visualize_when_busy']);
        ...
        if (!saved_prev_obj_keys.includes(objID)) {
            ...
            // any new victims need to be added to the victim dropdowns
            if ( obj.hasOwnProperty('class_inheritance') && obj['class_inheritance'].includes('Victim') ) {
//                console.log("Found new victim:", objID)
                add_victim_to_dropdown(objID);
            }
        ...
        // regenerate the specfic object shape with its settings. Also regerenate the score every tick
        if (style_object || obj['class_inheritance'].includes('Score')  || objID == "explorer") {
            set_tile_dimensions(obj_element);

            var shape = null;
            // draw the object with the correct shape, size and colour
            if (obj_vis_settings['img'] != null) {
                shape = gen_image(obj_vis_settings, obj_element);
            } else if (obj_vis_settings['shape'] == 0) {
                shape = gen_rectangle(obj_vis_settings, obj_element);
            } else if (obj_vis_settings['shape'] == 1) {
                shape = gen_triangle(obj_vis_settings, obj_element);
            } else if (obj_vis_settings['shape'] == 2) {
                shape = gen_circle(obj_vis_settings, obj_element);
            }

            // add text to an object, specific to the AIMS usecase
            obj_vis_settings = aims_add_object_text(objID, obj_vis_settings, obj, shape);
        }
...
/*
 * Add (aims specific) text to an object
 */
function aims_add_object_text(objID, obj_vis_settings, obj, shape) {
    var text_element = document.createElement("div");

    // Add room number and status
    if(obj['class_inheritance'].includes('RoomAgent')){

        var text_element_content = "";

        // add room name
        if(obj['name'] == 'command_post'){
            text_element_content = "Commandopost ";
        } else{
            text_element_content = obj['name'] + " ";
        }

        // add room status
        if(obj['collapsed'] != 'unknown'){
            if(obj['collapsed']){
                text_element_content += "Ingestort";
                obj_vis_settings['building_status'] = "ingestort";
            }else{
                text_element_content += "Niet ingestort";
                obj_vis_settings['building_status'] = "niet-ingestort";
            }
        } else {
            obj_vis_settings['building_status'] = "unknown";
        }


        // add to the html
        text_element.innerHTML = text_element_content;
        text_element.classList.add('aims-room-number');
        text_element.style.fontSize = (tile_size * 0.5 )+ "px";
        shape.appendChild(text_element);


    // Add victim number to victims
    } else if(obj['class_inheritance'].includes('Victim')){
        text_element.innerHTML = obj['name'].replace('victim', '');
        text_element.classList.add('aims-victim-number');
        text_element.style.fontSize = (tile_size * 0.5 * 0.8)+ "px";
        shape.parentNode.appendChild(text_element);


    // Add energy level above explorer
    } else if(obj['class_inheritance'].includes('Explorer')){
        text_element.innerHTML = obj['energy'];
        text_element.classList.add('aims-battery-level');
        obj_vis_settings['battery_status'] = obj['energ'];
        text_element.style.fontSize = (tile_size * 0.5 * 0.8)+ "px";
        shape.parentNode.appendChild(text_element);


    // Draw score
    } else if(obj['class_inheritance'].includes('Score')){
        text_element.classList.add('aims-score');
        text_element.id = "aims-score";

        // remove previous score object, if any
        var score_element = document.getElementById("aims-score");
        if (score_element) {
            score_element.remove();
        }

        // by default the score is placed right, place left if requested
        if (obj['score_vis_placement'] == "left") {
            text_element.classList.add('aims-score-left');
        }

        // list all scores
        if(obj['victims_total'] != ''){
            text_element.appendChild(document.createTextNode('Aantal accu’s vervangen: ' + obj['batteries_replaced']));
            text_element.appendChild(document.createElement("br"));
            text_element.appendChild(document.createTextNode('Slachtoffers totaal: ' + obj['victims_total']));
            text_element.appendChild(document.createElement("br"));
            text_element.appendChild(document.createTextNode('Slachtoffers levend: ' + obj['victims_alive']));
            text_element.appendChild(document.createElement("br"));
        }
        text_element.appendChild(document.createTextNode('Slachtoffers gered: ' + obj['victims_retrieved']));
        text_element.appendChild(document.createElement("br"));
        text_element.appendChild(document.createTextNode('Verstreken tijd: ' + obj['ticks_elapsed']));

        // add score to html
        text_element.style.fontSize = (tile_size * 0.5 * 0.8)+ "px";
        document.getElementById("grid").appendChild(text_element);
    }

    return obj_vis_settings;
}


/**
 * Add a victim to all victim dropdown(s)
 */
function add_victim_to_dropdown(objID) {
    console.log("Adding new victim ", objID, " to all dropdowns");

    // update all dropdowns
    var dropdowns = document.querySelectorAll(".victim_dropdown");
    dropdowns.forEach(function(dropdown) {

        // add the new victim as a new option
...
function fix_grid_size() {
    ...
    if (fix_tile_size()) {
        ...
        document.getElementById("chat").style.marginLeft = grid.style.width;
    }
...
function gen_rectangle(obj_vis_settings, obj_element, element_type = "div") {
    ...
    if (obj_vis_settings['busy']) {
        ...
        busy_icon.setAttribute("src", '/images/busy_black_small.gif');
        // small loading icon
        busy_icon.style.width = 0.4 * tile_size + "px";
        busy_icon.style.height = 0.4 * tile_size + "px";
        // position in the top left corner
        busy_icon.style.right = -0.2 * tile_size + "px";
        busy_icon.style.top = -0.1 * tile_size + "px";
...
/*
 * Show the completion screen with any extra data such as scores
 */
function show_completion_screen(completion_data) {

    // check if we already added the completion screen
    if (document.getElementById("completion-screen") != null) {
        return;
    };

    console.log(completion_data)

    // create a completion element
    var completion_ribbon = document.createElement("div");
    completion_ribbon.className = "completion-ribbon";
    completion_ribbon.id = "completion-screen"

    // add the completion text
    var completion_text = document.createElement("div");
    completion_text.className = "completion-text"
    completion_text.appendChild(document.createTextNode(completion_data.world_completion_message));
    completion_ribbon.appendChild(completion_text);

    // add to the grid
    grid.append(completion_ribbon);

    console.log("added completion data to GUI")
}
